...
const STATUS_DOT = {
 ...
 failed: "bg-red-500",
 pending: "bg-background-secondary",
 cancelled: "bg-background-secondary",
};
...
export default function CalendarPage() {
 ...
 <button
 onClick={() => goToMonth(-1)}
 className="w-9 h-9 rounded-lg border border-surface-border flex items-center justify-center hover:bg-background-secondary transition-colors"
 >
 ...
 <button
 onClick={goToToday}
 className="px-3 h-9 rounded-lg border border-surface-border text-sm font-medium hover:bg-background-secondary transition-colors"
 >
 Today
 </button>
 <button
 ...
 {error && (
 <div className="bg-red-500/10 border border-red-500/20 text-red-600 text-sm mb-4 py-3 px-4 rounded-xl">
 {error}
 </div>
 )}
 ...
 {WEEKDAYS.map((w) => (
 <div key={w} className="text-center text-xs font-semibold text-foreground-muted py-1.5">
 {w}
 </div>
 ))}
 </div>
 ...
 <div key={i} className="aspect-square rounded-xl bg-background-secondary animate-pulse" />
 ))}
 </div>
 ) : (
 <div className="grid grid-cols-7 gap-1.5">
 ...
 isToday ? "bg-brand-600 text-white" : "text-foreground-subtle "
 }`}
 >
 {day.getDate()}
 </span>
 ...
 <span
 key={p.id}
 className={`w-full h-1.5 rounded-full ${STATUS_DOT[p.status] || "bg-background-secondary"}`}
 />
 ))}
 {dayPosts.length > 2 && (
 ...
 <svg className="w-10 h-10 text-foreground-muted mx-auto mb-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
 ...
 <div
 key={post.id}
 className="p-3.5 rounded-xl border border-surface-border bg-surface/40 "
 >
 ...
 className={`w-4 h-4 rounded-full ${PLATFORM_META[p]?.color || "bg-background-secondary"}`}
 />
 ))}
 </div>
 </div>
