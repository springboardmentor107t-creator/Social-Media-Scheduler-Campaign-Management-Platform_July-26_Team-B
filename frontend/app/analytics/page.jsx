...
export default function AnalyticsPage() {
  ...
  // Draw Line Chart helper using raw SVGs
  function renderSvgLineChart() {
    ...
    campaignAnalytics.forEach((record) => {
      ...
      if (!dateMap[dateStr]) {
        dateMap[dateStr] = { impressions: 0, likes: 0, clicks: 0, date: dateStr };
      }
    ...
    const maxVal = Math.max(...dataPoints.map(d => d.impressions)) || 1000;
    const minVal = 0;
      ...
      <svg className="w-full h-full" viewBox={`0 0 ${width} ${height}`} preserveAspectRatio="none">
        ...
        {[0, 0.25, 0.5, 0.75, 1].map((ratio) => {
          ...
          return (
            <g key={ratio} className="opacity-40">
              ...
              />
              <text x={padding - 10} y={y + 4} textAnchor="end" className="text-[10px] font-medium fill-slate-400">
                {val}
              </text>
            </g>
          );
        ...
        {dataPoints.map((pt, i) => (
          <circle
            key={i}
            cx={getX(i)}
            cy={getY(pt.impressions)}
            r={4}
            fill="#ffffff"
            stroke="#6366f1"
            strokeWidth={2}
            className="hover:r-6 cursor-pointer transition-all"
            title={`${pt.date}: ${pt.impressions} impressions`}
          />
        ))}

        {/* X Axis Labels */}
        {dataPoints.map((pt, i) => {
          // Label only every alternate item if too many points
  ...
  const inputClass =
    "w-full px-4 py-2.5 rounded-xl bg-white/50 dark:bg-zinc-900/50 border border-slate-200 dark:border-slate-800 focus:outline-none focus:ring-2 focus:ring-brand-500/50 focus:border-brand-500 transition-all placeholder:text-slate-400 dark:placeholder:text-slate-600 text-sm";
  ...
  return (
    <DashboardShell>
      <div className="space-y-6">
        ...
        {/* Alerts */}
        {error && (
          <div className="bg-red-500/10 border border-red-500/20 text-red-600 dark:text-red-400 text-sm py-3 px-4 rounded-xl flex items-center gap-2">
            {error}
          </div>
        )}
        {success && (
          <div className="bg-green-500/10 border border-green-500/20 text-green-700 dark:text-green-400 text-sm py-3 px-4 rounded-xl flex items-center gap-2">
            {success}
          </div>
        )}
        ...
        {loading ? (
          <div className="space-y-6">
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
              {[1, 2, 3, 4].map(i => <div key={i} className="h-28 bg-slate-100 dark:bg-zinc-900 rounded-2xl animate-pulse" />)}
            </div>
            <div className="h-80 bg-slate-100 dark:bg-zinc-900 rounded-3xl animate-pulse" />
          </div>
        ) : (
          <>
            {/* Overview cards */}
            {overview && (
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                <div className="glass-panel p-5 rounded-2xl">
                  <p className="text-xs font-bold uppercase tracking-wider text-slate-400">Total Impressions</p>
                  <p className="text-2xl font-bold text-slate-800 dark:text-slate-100 mt-1">{overview.total_impressions?.toLocaleString()}</p>
                </div>
                <div className="glass-panel p-5 rounded-2xl">
                  <p className="text-xs font-bold uppercase tracking-wider text-slate-400">Total Reach</p>
                  <p className="text-2xl font-bold text-slate-800 dark:text-slate-100 mt-1">{overview.total_reach?.toLocaleString()}</p>
                </div>
                <div className="glass-panel p-5 rounded-2xl">
                  <p className="text-xs font-bold uppercase tracking-wider text-slate-400">Engagement Actions</p>
                  <p className="text-2xl font-bold text-slate-800 dark:text-slate-100 mt-1">{(overview.total_likes + overview.total_comments + overview.total_shares + overview.total_clicks)?.toLocaleString()}</p>
                </div>
                <div className="glass-panel p-5 rounded-2xl">
                  <p className="text-xs font-bold uppercase tracking-wider text-slate-400">Avg. Engagement Rate</p>
                  <p className="text-2xl font-bold text-slate-800 dark:text-slate-100 mt-1">{overview.avg_engagement_rate}%</p>
                </div>
              </div>
            )}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="lg:col-span-2 glass-panel p-6 rounded-3xl space-y-4">
                <div className="flex items-center justify-between gap-4">
                  <div>
                    <h3 className="text-sm font-bold uppercase tracking-wider text-slate-500">Impressions Over Time</h3>
                    <p className="text-xs text-slate-400 mt-0.5">Campaign performance timeline trends</p>
                  </div>
                  <select
                    value={selectedCampaignId}
                    onChange={handleCampaignChange}
                    className="px-3 py-1.5 rounded-xl border border-slate-200 dark:border-slate-800 bg-white/50 dark:bg-zinc-900/50 text-xs font-semibold focus:outline-none"
                  >
                ...
                <div className="h-64 relative bg-white/30 dark:bg-zinc-950/20 rounded-2xl p-2 border border-slate-200/20 dark:border-slate-800/20">
                  {loadingChart ? (
              ...
              <div className="lg:col-span-1 glass-panel p-6 rounded-3xl flex flex-col justify-between">
                <div>
                  <h3 className="text-sm font-bold uppercase tracking-wider text-slate-500 mb-1">Platform Share</h3>
                  <p className="text-xs text-slate-400 mb-6">Distribution of campaign impressions by network</p>
                </div>
                ...
                {overview && Object.keys(overview.platform_breakdown).length > 0 ? (
                      ...
                      return (
                        <div key={platform} className="space-y-1.5">
                          <div className="flex justify-between items-center text-xs font-semibold">
                            ...
                            <span className="text-slate-400">{percent}% ({data.impressions.toLocaleString()} views)</span>
                          </div>
                          <div className="w-full h-3 bg-slate-200 dark:bg-zinc-800 rounded-full overflow-hidden">
                            <div
                              className={`h-full ${barColors[platform] || "bg-slate-400"}`}
                              style={{ width: `${percent}%` }}
                            />
                          </div>
                        </div>
                  ...
                  <p className="text-sm text-slate-400 text-center py-10">No platform breakdown data available.</p>
                )}
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              {audienceOverview && (
                <div className="lg:col-span-1 glass-panel p-6 rounded-3xl space-y-4">
                  <h3 className="text-sm font-bold uppercase tracking-wider text-slate-500">Audience Growth</h3>
                  <p className="text-xs text-slate-400">General network growth statistics</p>

                  <div className="grid grid-cols-2 gap-4 pt-2">
                    <div className="p-4 rounded-xl bg-white/40 dark:bg-zinc-900/40 border border-slate-200/50 dark:border-slate-800/50 text-center">
                      <p className="text-xs font-bold text-slate-400 uppercase">Estimated Reach</p>
                      <p className="text-xl font-bold text-slate-800 dark:text-slate-100 mt-1">{audienceOverview.total_estimated_reach?.toLocaleString()}</p>
                    </div>
                    <div className="p-4 rounded-xl bg-white/40 dark:bg-zinc-900/40 border border-slate-200/50 dark:border-slate-800/50 text-center">
                      <p className="text-xs font-bold text-slate-400 uppercase">Active Segments</p>
                      <p className="text-xl font-bold text-slate-800 dark:text-slate-100 mt-1">{audienceOverview.active_segments} / {audienceOverview.total_segments}</p>
                    </div>
                  </div>

                  <div className="space-y-3 pt-4 border-t border-slate-200/30 dark:border-slate-800/30">
                    <p className="text-xxs font-bold text-slate-400 uppercase">Segments by Primary Channel</p>
                    <div className="grid grid-cols-2 gap-2 text-xs">
                      {Object.entries(audienceOverview.segments_by_platform).map(([p, count]) => (
                        <div key={p} className="flex justify-between items-center py-1 border-b border-slate-200/10 dark:border-slate-800/10 font-semibold">
                          <span className="capitalize">{p}</span>
                          <span className="text-slate-400">{count} segments</span>
                        </div>
                      ))}
                    </div>
                  </div>
              ...
              {/* Segment CRUD */}
              <div className="lg:col-span-2 glass-panel p-6 rounded-3xl space-y-4">
                <h3 className="text-sm font-bold uppercase tracking-wider text-slate-500">Audience Segmentation</h3>
                <p className="text-xs text-slate-400">Divide followers into targeted customer segments to post matching campaigns.</p>

                {segments.length === 0 ? (
                  <div className="py-8 text-center border border-dashed border-slate-200 dark:border-slate-800 rounded-2xl">
                    <p className="text-xs text-slate-500">No audience segments defined.</p>
                  </div>
                ) : (
                  <div className="space-y-2.5 max-h-[280px] overflow-y-auto pr-1">
                    {segments.map((seg) => (
                      <div
                        key={seg.id}
                        className="flex items-center justify-between gap-4 p-3.5 border border-slate-200/50 dark:border-slate-800/50 rounded-xl bg-white/40 dark:bg-zinc-900/40"
                      >
                        <div>
                          <div className="flex items-center gap-2">
                            <h4 className="text-sm font-bold text-slate-800 dark:text-slate-100">{seg.name}</h4>
                            <span className="text-xxs px-2 py-0.5 bg-slate-100 dark:bg-zinc-800 text-slate-500 font-bold rounded-full capitalize">{seg.platform}</span>
                          </div>
                          <p className="text-xs text-slate-400 mt-0.5 line-clamp-1">{seg.description || "No description."}</p>
                          <p className="text-xxs text-slate-500 font-bold mt-1 uppercase">
                            Size: <span className="text-slate-700 dark:text-slate-300 font-extrabold">{seg.estimated_size?.toLocaleString()}</span> | Interests: <span className="text-slate-700 dark:text-slate-300 font-extrabold">{seg.interests?.join(", ")}</span>
                          </p>
                        </div>
                        <button
        ...
        {showSegmentModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
            <div className="glass-panel w-full max-w-lg rounded-3xl p-6 sm:p-8 bg-white dark:bg-zinc-950 shadow-2xl relative" onClick={e => e.stopPropagation()}>
              <button
              ...
              <p className="text-xs text-slate-400 mb-6">Define a targeted user profile demographics for campaign coordination.</p>
              ...
              <form onSubmit={handleCreateSegment} className="space-y-4">
                ...
                <div className="pt-4 flex items-center justify-end gap-3">
                  <button
                    type="button"
                    onClick={() => setShowSegmentModal(false)}
                    className="px-4 py-2 rounded-xl text-sm font-semibold text-slate-500 hover:bg-slate-100 dark:hover:bg-zinc-800 transition-colors"
                  >
                    Cancel
                  </button>
                  <button
