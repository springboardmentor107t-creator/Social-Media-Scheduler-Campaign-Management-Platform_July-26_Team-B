...
export default function CreatePostPage() {
 ...
 const inputClass =
 "w-full px-4 py-3 rounded-xl bg-surface border border-surface-border focus:outline-none focus:ring-2 focus:ring-brand-500/50 focus:border-brand-500 transition-all placeholder:text-foreground-muted ";
 ...
 content.length > shortestLimit ? "text-red-500" : "text-foreground-muted"
 }`}
 >
 ...
 <div key={i} className="relative w-20 h-20 rounded-xl overflow-hidden border border-surface-border group">
 {media.type === "image" ? (
 ...
 <label className="w-20 h-20 rounded-xl border-2 border-dashed border-surface-border flex flex-col items-center justify-center cursor-pointer hover:border-brand-500 transition-colors">
 <svg className="w-6 h-6 text-foreground-muted" fill="none" viewBox="0 0 24 24" stroke="currentColor">
 <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 4v16m8-8H4" />
 </svg>
 <span className="text-xs text-foreground-muted mt-1">Add</span>
 <input
 type="file"
 accept="image/*,video/*"
 multiple
 ...
 {loadingAccounts ? (
 <p className="text-sm text-foreground-muted">Loading connected accounts...</p>
 ) : (
 ...
 <div className="grid grid-cols-3 gap-2 p-1 rounded-xl bg-background-secondary border border-surface-border ">
 {MODES.map((m) => (
 <button
 key={m.value}
 type="button"
 ...
 {/* Schedule fields */}
 {mode === "schedule" && (
 <div className="space-y-4 p-4 rounded-xl bg-background-secondary border border-surface-border ">
 <div className="grid grid-cols-2 gap-4">
 <div>
 <label className="block mb-1.5 text-xs font-semibold text-foreground-subtle ">
 Date
 </label>
 <input
 type="date"
 ...
 <div
 key={p}
 className="p-4 rounded-2xl border border-surface-border bg-surface/40 "
 >
