...
function StatCard({ label, value, icon, loading }) {
  return (
    <div className="glass-panel p-5 rounded-2xl">
      ...
      {loading ? (
        <div className="h-7 w-12 bg-slate-200 dark:bg-slate-800 rounded animate-pulse mb-1" />
      ) : (
        <p className="text-2xl font-bold text-slate-800 dark:text-slate-100 mb-0.5">
          {value}
        </p>
      )}
            ...
            <div className="space-y-2">
              {quickActions.map((action) => (
                <button
                  ...
                  className="w-full flex items-center gap-3 px-4 py-3 rounded-xl border border-slate-200 dark:border-slate-800 hover:bg-slate-100 dark:hover:bg-zinc-800 transition-colors text-left"
                >
          ...
          <div className="lg:col-span-2 glass-panel p-6 rounded-3xl">
            ...
            {loading ? (
              <div className="space-y-3">
                {[1, 2, 3].map((i) => (
                  <div key={i} className="h-12 bg-slate-100 dark:bg-zinc-900 rounded-xl animate-pulse" />
                ))}
              </div>
            ) : recent.length === 0 ? (
              ...
              <div className="space-y-2.5">
                {recent.map((post) => (
                  <div
                    key={post.id}
                    className="flex items-center gap-3 p-3 rounded-xl border border-slate-200 dark:border-slate-800 bg-white/40 dark:bg-zinc-900/40"
                  >
