    ...
    return (
      <DashboardShell>
        <div className="glass-panel p-8 rounded-3xl text-center max-w-md mx-auto">
          ...
          <p className="text-sm text-slate-500 mb-6">The campaign you are looking for does not exist or you do not have permission to view it.</p>
  ...
  return (
    <DashboardShell>
      <div className="space-y-6">
        ...
        {/* Back Link */}
        <Link href="/campaigns" className="flex items-center gap-1.5 text-xs font-semibold text-slate-500 hover:text-brand-500 transition-colors w-fit">
        ...
        <div className="glass-panel p-6 sm:p-8 rounded-3xl space-y-6">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-6 pt-4 border-t border-slate-200/30 dark:border-slate-800/30">
            <div>
              <p className="text-xs font-bold uppercase tracking-wider text-slate-400 mb-1">Budget Allocation</p>
              <p className="text-xl font-bold text-slate-800 dark:text-slate-100">${campaign.budget?.toLocaleString() || "0"}</p>
            </div>
            <div>
              <p className="text-xs font-bold uppercase tracking-wider text-slate-400 mb-1">Target Audience</p>
              <p className="text-sm font-semibold text-slate-700 dark:text-slate-200 truncate">{campaign.target_audience || "General"}</p>
            </div>
            <div>
              <p className="text-xs font-bold uppercase tracking-wider text-slate-400 mb-1">Content Items</p>
              <p className="text-xl font-bold text-slate-800 dark:text-slate-100">{contents.length}</p>
            </div>
            <div>
              <p className="text-xs font-bold uppercase tracking-wider text-slate-400 mb-1">Date Created</p>
              <p className="text-sm font-semibold text-slate-700 dark:text-slate-200">{new Date(campaign.created_at).toLocaleDateString()}</p>
            </div>
          </div>
        </div>
        ...
        {analytics.length > 0 && (
          <div className="grid grid-cols-2 lg:grid-cols-5 gap-4">
            <div className="glass-panel p-4 rounded-2xl text-center">
              ...
              <p className="text-lg font-bold text-slate-800 dark:text-slate-100 mt-1">{totalImpressions.toLocaleString()}</p>
              <p className="text-xxs font-bold uppercase tracking-wider text-slate-400">Impressions</p>
            </div>
            <div className="glass-panel p-4 rounded-2xl text-center">
              <span className="text-lg">👥</span>
              <p className="text-lg font-bold text-slate-800 dark:text-slate-100 mt-1">{totalReach.toLocaleString()}</p>
              <p className="text-xxs font-bold uppercase tracking-wider text-slate-400">Reach</p>
            </div>
            <div className="glass-panel p-4 rounded-2xl text-center">
              <span className="text-lg">❤️</span>
              <p className="text-lg font-bold text-slate-800 dark:text-slate-100 mt-1">{totalLikes.toLocaleString()}</p>
              <p className="text-xxs font-bold uppercase tracking-wider text-slate-400">Likes</p>
            </div>
            <div className="glass-panel p-4 rounded-2xl text-center">
              <span className="text-lg">💬</span>
              <p className="text-lg font-bold text-slate-800 dark:text-slate-100 mt-1">{totalComments.toLocaleString()}</p>
              <p className="text-xxs font-bold uppercase tracking-wider text-slate-400">Comments</p>
            </div>
            <div className="glass-panel p-4 rounded-2xl text-center col-span-2 lg:col-span-1">
              <span className="text-lg">🖱️</span>
              <p className="text-lg font-bold text-slate-800 dark:text-slate-100 mt-1">{totalClicks.toLocaleString()}</p>
              <p className="text-xxs font-bold uppercase tracking-wider text-slate-400">Clicks</p>
            </div>
          </div>
        )}
        ...
        <div className="glass-panel p-6 sm:p-8 rounded-3xl">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
            ...
            <div className="flex gap-2">
              <button
                ...
                className="border border-slate-200 dark:border-slate-800 hover:bg-slate-100 dark:hover:bg-zinc-800 px-4 py-2 rounded-xl text-xs font-semibold transition-all"
              >
                🔗 Link Existing Post
              </button>
              <button
          ...
          {contents.length === 0 ? (
            <div className="py-12 text-center">
              ...
              <h4 className="text-sm font-bold text-slate-700 dark:text-slate-200 mb-1">No content linked yet</h4>
              <p className="text-xs text-slate-500 max-w-sm mx-auto mb-4">You haven't added or linked any posts to this campaign. Keep your strategy organized by adding scheduled posts.</p>
            </div>
          ) : (
            <div className="space-y-3">
              {contents.map((item) => (
                <div
                  key={item.id}
                  className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-4 border border-slate-200/50 dark:border-slate-800/50 rounded-2xl bg-white/40 dark:bg-zinc-900/40 hover:bg-slate-50/50 dark:hover:bg-zinc-900/60 transition-all"
                >
                  <div className="flex items-start gap-3 min-w-0">
                    <div className="w-10 h-10 rounded-xl bg-brand-500/10 flex items-center justify-center font-bold text-brand-600 dark:text-brand-400 text-sm shrink-0">
                      {item.channel?.slice(0, 2).toUpperCase()}
                    </div>
                    <div className="min-w-0">
                      <h4 className="text-sm font-bold text-slate-800 dark:text-slate-100 tracking-tight leading-tight truncate">
                        {item.title}
                      </h4>
                      <p className="text-xxs text-slate-400 mt-1 uppercase font-semibold">
                        Platform: <span className="text-slate-500 dark:text-slate-300 font-bold">{item.channel}</span> | Type: <span className="text-slate-500 dark:text-slate-300 font-bold">{item.content_type}</span>
                      </p>
                      {item.scheduled_time && (
        ...
        {showAddContentModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
            <div className="glass-panel w-full max-w-lg rounded-3xl p-6 sm:p-8 bg-white dark:bg-zinc-950 shadow-2xl relative">
              <button
              ...
              <p className="text-xs text-slate-400 mb-6">Create and schedule a new post directly in this campaign.</p>
        ...
        {showLinkPostModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
            <div className="glass-panel w-full max-w-lg rounded-3xl p-6 sm:p-8 bg-white dark:bg-zinc-950 shadow-2xl relative">
              ...
              <p className="text-xs text-slate-400 mb-6">Choose an already created post to include in this campaign.</p>
