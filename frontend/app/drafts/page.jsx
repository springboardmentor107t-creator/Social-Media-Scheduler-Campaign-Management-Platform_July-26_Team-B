      ...
      <div className="glass-panel p-8 rounded-3xl">
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-16 text-center">
            ...
            <h3 className="font-semibold text-slate-700 dark:text-slate-200 mb-1">No drafts yet</h3>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {drafts.map((draft) => (
              <div
                key={draft.id}
                className="p-5 rounded-2xl border border-slate-200 dark:border-slate-800 bg-white/40 dark:bg-zinc-900/40 hover:shadow-md transition-all flex flex-col"
              >
                <p className="text-sm text-slate-700 dark:text-slate-300 line-clamp-3 mb-4 min-h-[3.75rem]">
                ...
                <div className="flex items-center gap-1.5 mb-4">
                  {(draft.platforms || []).map((p) => (
                    <span
                      ...
                      className={`w-5 h-5 rounded-full ${PLATFORM_META[p]?.color || "bg-slate-400"}`}
                    />
                  ))}
                </div>
                ...
                {schedulingId === draft.id ? (
                  <div className="space-y-2 mt-auto">
                    <div className="grid grid-cols-2 gap-2">
                      <input
                        ...
                        className="px-2 py-1.5 text-xs rounded-lg bg-white dark:bg-zinc-900 border border-slate-200 dark:border-slate-800"
                      />
                      <input
                        type="time"
                        value={scheduleTime}
                    ...
                    <div className="flex gap-2">
                      ...
                      <button
                        ...
                        className="flex-1 py-1.5 rounded-lg border border-slate-200 dark:border-slate-800 text-xs font-medium"
                      >
                        Cancel
                      </button>
                    </div>
                  ...
                  <div className="flex gap-2 mt-auto">
                    <button
                      ...
                      className="flex-1 py-1.5 rounded-lg border border-slate-200 dark:border-slate-800 text-xs font-medium hover:bg-slate-100 dark:hover:bg-zinc-800 transition-colors"
                    >
                      Edit
                    </button>
                    <button
                    ...
                    <button
                      ...
                      className="px-3 py-1.5 rounded-lg text-red-500 hover:bg-red-50 dark:hover:bg-red-950/30 text-xs font-medium transition-colors"
                    >
                      Delete
                    </button>
                  </div>
